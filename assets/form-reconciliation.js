(function initFormReconciliation(){
  ...
  function feedbackBreakdown(sum,label,key){
    const counts={};sum.responses.forEach(function(r){const value=C.clean(r.feedback&&r.feedback[key])||'Not stated';counts[value]=(counts[value]||0)+1;});
    return '<section class="form-feedback-breakdown"><h4>'+esc(label)+'</h4><div>'+Object.keys(counts).sort(function(a,b){return counts[b]-counts[a]||a.localeCompare(b);}).map(function(value){return'<span><b>'+esc(value)+'</b> '+counts[value]+'</span>';}).join('')+'</div></section>';
  }
  ...
  async function writeAttendance(){
    ...
    for(const s of state.sessions){
      if(s.included===false)continue;const source=sessionSource(s);if(!source)continue;
      let existing=attendanceCandidate(rows,s),signInAt=s.signIn&&s.signIn.timestamp.iso||null,signOutAt=s.signOut&&s.signOut.timestamp.iso||null,calculated=s.calculatedMinutes,credited=s.staffCreditedMinutes;
      if(existing){signInAt=signInAt||existing.sign_in_at||null;signOutAt=signOutAt||existing.sign_out_at||null;if(calculated==null&&signInAt&&signOutAt){const delta=Math.round((Date.parse(signOutAt)-Date.parse(signInAt))/60000);if(Number.isFinite(delta)&&delta>=0&&delta<=30*60)calculated=delta;}if(credited==null&&existing.staff_credited_duration_minutes!=null)credited=Number(existing.staff_credited_duration_minutes);}
      const finalMinutes=credited==null?(calculated==null?(existing?Number(existing.duration_minutes)||0:0):calculated):credited;
      const body={volunteer_id:s.volunteerMatch&&s.volunteerMatch.id||existing&&existing.volunteer_id||null,attended:true,event_name:s.eventName,event_date:s.eventDate,duration_minutes:finalMinutes,sign_in_at:signInAt,sign_out_at:signOutAt,calculated_duration_minutes:calculated,staff_credited_duration_minutes:credited,staff_credit_note:s.staffCreditNote||existing&&existing.staff_credit_note||null,form_reconciliation_id:s.id};
